...
def inline_fn(str, style = :pandoc)
  ...
  while (ref_start = text.match(/\[\^(\d+)\]:/))
    counter += 1
    cite = "[^#{counter}]"
    ref_end = text.match(/\[\^(\d+)\]:/m, ref_start.end(0))&.begin(0).to_i - 2
    ...
    text[ref_start.begin(0)..ref_end] = "#{cite}#{text[ref_start.end(0)..ref_end].strip}#{note}"
  end
