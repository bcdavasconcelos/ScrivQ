...
#let make-index-int(title: none, main-color-state:none) = {
    ...
    let content-text(content) = {
        let ct = ""
        if content.has("text") {ct = content.text}
        else {
            for cc in content.children {
                if cc.has("text") {ct += cc.text}
            }
        }
        return ct
    }
    ...
    context{
        ...
        for el in elements {
            let ct = ""
            if el.caption.has("body"){ct = content-text(el.caption.body)}
            else{ct = content-text(el.caption)}

            // Have we already know that entry text? If not,
            // add it to our list of entry words
            if words.keys().contains(ct) != true {words.insert(ct, ())}

            // Add the new page entry to the list.
            let ent = (class: el.body.text, page: el.location().page())
            if not words.at(ct).contains(ent){words.at(ct).push(ent)}
        }
        ...
        let register = ""
        if title != none {heading(level: 1, numbering: none, title)}
        block(columns(2,gutter: 1cm, [
            #for sk in sortedkeys [
                #let formattedPageNumbers = words.at(sk).map(en => {link((page: en.page, x:0pt, y:0pt), text(fill: black, str(en.page)))})
