...
[{#tbl-scriv154} This is native Scrivener table with a referenced table caption. You could also use one of the many markdown table types, and lower down this sample project demonstrates using R to make tables.]
...
![Elephant castle.](Elephant2.jpg){#fig-castle width=243 height=251}

![Angry elephant with big trunk.](Elephant3.jpg){#fig-trunk width=304 height=248}
...
![This figure uses custom metadata values to identify the class, ID, width and height. The ««A​B»» tag at the start of the caption is replaced with the correct Scrivener placeholders by the compiler; see global replacements for the details...](Ulysses1.jpg){#fig-scriv167 .column-page width=537 height=444}
...
:::{.column-page-right}
![This should span the page to the right in HTML. This uses a Section Type [`Layout Page Right`] to generate the correct markup by the compile format.](Elephant3.jpg){#fig-scriv170 width=506 height=411}
...
![Elephant.](Elephant2.jpg){#fig-castle2 width=245 height=252}

![Angry elephant with big trunk.](Elephant3.jpg){#fig-trunk2 width=306 height=249}
...
![The new layout makes it easier editing parameters without disturbing YAML's sensitive white space rules.](yaml.png){#fig-crossref width=536 height=450}
...
To reference them, use `scrivautο`, `%autοref%`, `[autοref]` or simply `scriv` &  `<\$linkID>` with the proper style applied; that is, [@cnj-scriv183], [@cor-scriv183], [@def-scriv183], [@exm-scriv183], [@exr-scriv183], [@lem-scriv183], [@prp-scriv183], [@thm-scriv183]. **Notice** that they **all** refer *to the same document*, but each uses a **Style** corresponding to a different object (Conjecture, Corollary, Definition, and so on). The caveat here is that we **_cannot_** have more than [one]{.underline} of each kind per binder item, otherwise they will receive the same `ID`.
...
: Simple example of how we are using styles to create the correct markup {#tbl-scriv203  }
  

[4. Added]{.smallcaps} the prefix `[]{#scriv\<$linkID>}` to some **Section Layouts**. 


This allows us to create links to [text sections that do not have a Header element](#scriv204).   
...
![Revising code in Scrivener can be just as pleasant](scrivenerwithcode.png){#fig-scriv205 width=554 height=346}
...
![Students finishing their MDs or PhDs will certainly appreciate the possibility of adding your own latex customizations, which can also be done by including the code within certain fields, such as before-body.](pandocquartolatextemplate.png){#fig-scriv206 .column-page width=547 height=342}


  
:::{#%autoref% .callout-important }
...
# Primary Sources {#sec-scriv210 .appendix}
...
# Secondary Sources {#sec-scriv211 .appendix}
...
# Workflow {#sec-scriv212 .appendix}

---
bibliography_scriv212: |
  refs/workflow.bib
---
