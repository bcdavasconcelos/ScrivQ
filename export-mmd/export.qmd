...
Sint meis quo et, vis ad fæcete dolorem! Ad quøt moderatius elaboraret eum[@crivellato2007], pro paulo ridens quaestio ut (see [@fig-scriv144])! Iudico nullam sit ad, ad has åperiam senserit conceptåm? Tritani posidonium suscipiantur ex duo, meæ essent mentitum ad. Nåm ex mucius mandamus, ut duo cåusae offendit laboramus. Duo iisque sapientem ad, vølumus persecuti vix cu, **_his åt justo putant comprehensam (this style is strong emphasis)_**.  
...
No meæ menandri mediøcritatem, meis tibique convenire vis id! Delicata intellegam mei ex. His consulåtu åssueverit ex, ei ius apeirian cønstituam mediocritatem, mei rebum detracto scaevølæ ex. Sed modo dico ullum at, sententiae definiebas ex eam! Nøstro eruditi eum ex.  See [@tbl-scriv144] for more details.   
...
: This is native Scrivener table with a referenced table caption. You could also use one of the many markdown table types, and lower down this sample project demonstrates using R to make tables.  {#tbl-scriv144}
...
# Methods {#sec-scriv145}

## Data Recording {#sec-scriv146}
...
:::{.column-margin}
![A figure of a poor, poor marginalised elephant...](Elephant3.jpg){#fig-scriv146}
:::
...
See both [@eq-scriv146] and [@eq-scriv147] for more details:

$$t' = \frac{t - \dfrac{v}{c^{2}}x}{\sqrt{1 - \dfrac{v^{2}}{c^{2}}}}$$ {#eq-scriv146}

  
Sint meis quo et, vis ad fæcete dolorem!

$$\nabla \times \mathbf {H} ={\frac {1}{c}}\left(4\pi \mathbf {J} _{\text{f}}+{\frac {\partial \mathbf {D} }{\partial t}}\right)$$ {#eq-scriv147}
...
## Experimental Perturbations {#sec-scriv148}
...
This next part will demonstrate the use of raw markdown within the document to create a multipart figure. See [@fig-scriv164 ]below for an example using a Section Type to insert the same markup at compile-time.
...
## Stimulus Plotting {#sec-scriv149}
...
Here is an R plot ([@fig-scriv149]), you need to have R installed for this to work, if not remove this document from the compile:  
...
## Statistical Analysis {#sec-scriv152}
...
Sint meis quo et, vis ad fæcete dolorem! Ad quøt moderatius elaboraret eum, pro paulo ridens quaestio ut! Iudico nullam sit ad, ad has åperiam senserit conceptåm? Tritani posidonium suscipiantur ex duo, meæ essent mentitum ad. Nåm ex mucius mandamus, ut duo cåusae offendit laboramus. Duo iisque sapientem ad, vølumus persecuti vix cu, his åt justo putant comprehensam. See [@fig-scriv153] and [@fig-scriv155] for details.
...
Sint meis quo et, vis ad fæcete dolorem! Ad quøt moderatius elaboraret eum, pro paulo ridens quaestio ut! Iudico nullam sit ad, ad has åperiam senserit conceptåm? Tritani posidonium suscipiantur ex duo, meæ essent mentitum ad. Nåm ex mucius mandamus, ut duo cåusae offendit laboramus. Duo iisque sapientem ad, vølumus persecuti vix cu, his åt justo putant comprehensam. See [@fig-scriv157] for details.  
...
# Results {#sec-scriv158}

## Lunar Cycles {#sec-scriv159}
...
Åd nam omnis ullamcørper vituperatoribus. Sed verear tincidunt rationibus an. Elit såperet recteque sit et, tåmquåm noluisse eloquentiåm ei mei. In pri solet soleat timeam, tale possit vis æt.  Please refer to [@tbl-scriv162], including [@tbl-first2] and [@tbl-second2] for more details.
  
:::{#tbl-scriv162   layout-ncol=1}
...
:::{#fig-scriv164 .column-page layout-ncol=2 layout-valign="bottom"}
...
$$\frac{d}{dx}\left( \int_{a}^{x} f(u)\,du\right)=f(x).$$ {#eq-scriv167}
...
# Acknowledgments {#sec-scriv169 .appendix  }
...
## Cross-referencing {#sec-scriv172}
...
### cnj, cor, def, exm, exr, lem, prp, thm {#sec-scriv173}


[]{#scriv173}


To reference them, use `scrivautο`, `%autοref%`, `[autοref]` or simply `scriv` &  `<\$linkID>` with the proper style applied; that is, [@cnj-scriv173], [@cor-scriv173], [@def-scriv173], [@exm-scriv173], [@exr-scriv173], [@lem-scriv173], [@prp-scriv173], [@thm-scriv173]. **Notice** that they **all** link *to the same document*, but each uses a **Style** corresponding to a different object (Conjecture, Corollary, Definition, and so on). The caveat here is that we **_cannot_** have more than [one]{.underline} of each kind per binder item, otherwise they will receive the same `ID`.
...
As we mentioned, you can also create environments using **Section Types** instead of styles. See [@cnj-scriv174], [@cor-scriv175], [@def-scriv176], [@exm-scriv177], [@exr-scriv178], [@lem-scriv179], [@prp-scriv180], [@thm-scriv181].
...
$$t' = \frac{t - \dfrac{v}{c^{2}}x}{\sqrt{1 - \dfrac{v^{2}}{c^{2}}}}$$ {#eq-scriv182}

```{#lst-scriv182 .ruby lst-cap="Ruby code block"}
puts "Quarto rocks"
```
...
: Table sample {#tbl-scriv182}


And see also [@eq-scriv183], [@tbl-scriv184].  
$$Equation content
$$ {#eq-scriv183}
...
: The caption {#tbl-scriv184  }

## CiteTools Integration {#sec-scriv185}
...
: Simple example of how we are using styles to create the correct markup {#tbl-scriv192  }
  

[4. Added]{.smallcaps} the prefix `[]{#scriv\<$linkID>}` to some **Section Layouts**. 


This allows us to create links to [text sections that do not have a Header element](#scriv193).   
...
:::{.callout-tip #scriv197  }
Right click on any Binder item. `Change Icon > Manage Custom Icons...`
:::

# Primary Sources {#sec-scriv198 .appendix}

---
bibliography_scriv198: |
...
# Secondary Sources {#sec-scriv199 .appendix}

---
bibliography_scriv199: |
...
# Workflow {#sec-scriv200 .appendix}
...
[Ulysses1]: Ulysses1.jpg  {#fig-scriv144 .column-page width=486 height=402}

[Elephant2]: Elephant2.jpg  {#fig-castle width=243 height=251}

[Elephant3]: Elephant3.jpg  {#fig-trunk width=304 height=248}

[Ulysses1-1]: Ulysses1.jpg  {#fig-scriv157 .column-page width=537 height=444}

[Elephant1]: Elephant1.jpg  {#fig-elespan width=520 height=262}

[Elephant3-1]: Elephant3.jpg  {#fig-scriv160 width=506 height=411}
...
[scrivenerwithcode]: scrivenerwithcode.png  {#fig-scriv194 width=554 height=346}

[pandocquartolatextemplate]: pandocquartolatextemplate.png  {#fig-scriv195 .column-page width=547 height=342}
